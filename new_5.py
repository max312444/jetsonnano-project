...
try:
    ...
    plt.hist(df[angle_column], bins=30, color='blue', alpha=0.7, edgecolor='black')
    ...
    # 데이터 초기화
    data = df[['frame_path', angle_column]].values.tolist()
    current_index = 0

    def load_image_and_angle(index):
        "이미지와 조향각을 로드"
        frame_path, angle = data[index]
        absolute_path = os.path.join(r"C:\Users\USER\dataset_0117", frame_path)
