...
frame_save_path = os.path.join(base_path, "csv") # 원본 이미지 경로
csv_file = r"C:\Users\USER\data.csv"  # CSV 파일 경로
...
def load_image_and_angle(index):
    ...
    absolute_path = os.path.join(base_path, frame_path)  # 절대 경로 생성
    image = cv2.imread(absolute_path)
    if image is None:
        print(f"이미지를 로드할 수 없습니다: {absolute_path}")
        return None, None
...
def delete_current_data(index):
    ...
    absolute_path = os.path.join(base_path, frame_path)
    if os.path.exists(absolute_path):
        os.remove(absolute_path)  # 이미지 파일 삭제
        print(f"Deleted Image: {absolute_path}")
